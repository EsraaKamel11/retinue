...
Scope, settled at brainstorm (2026-08-30, advisor-confirmed): the mint-validate-consume path
...
resolution mints zero or one token, the queue's insert-only shape stays untouched, and the
token's lifecycle is the new store's own contract. Two halves, one contract test suite, the
...
False and is refused. Append-only stores stay append-only; nothing is updated anywhere.
...
free retry riding an old approval. A send without a token, or with a token that fails any leg
